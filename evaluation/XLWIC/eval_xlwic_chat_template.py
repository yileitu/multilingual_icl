...
from util.func import extract_binary_response, load_eval_data_helper, load_model_tokenizer_pipeline, \
	prepend_random_sentences, retrieve_datapoint_by_id, get_low_high_resource_langs, sample_all_indices, \
	sample_random_sentences
from util.const import NO_SYS_PROMPT_MODEL_TYPES, NO_SYS_PROMPT_WORKAROUND
from data.XLWIC.util import SYS_PROMPT_COMMON, XLWIC_MAX_NEW_TOKENS, XLWIC_LANGS, XLWIC_USER_CONTENT_MAP
...
def construct_user_content(example, args: EvalArguments) -> str:
	...
	:param example: example datapoint
	:param args: evaluation arguments
	:return: User content string
	"""
	sentence1 = example["example_1"]
	sentence2 = example["example_2"]
	target_word = example["target_word"]
	shot_lang = example["language"]
	user_content_template = XLWIC_USER_CONTENT_MAP[shot_lang] \
		if args.all_source_language else XLWIC_USER_CONTENT_MAP['en']

	return user_content_template.format(sentence1=sentence1, sentence2=sentence2, target_word=target_word)
...
def construct_chat_messages(population: DatasetDict, args: EvalArguments, model_type: str, sampled_indices: List[int],
	...
	if args.cot_mode == 'direct':
		for i, example in enumerate(sampled_shots):
			user_content = construct_user_content(example=example, args=args)
			...
			label = example["label"]
			assistant_content = "Yes" if label == 1 else "No"
			# assistant_content = str(label)
...
def evaluate_language_split(model_pipeline: Pipeline | OpenAI, tokenizer: PreTrainedTokenizerFast,
	...
	for i, datapoint in enumerate(tqdm(eval_args.data_split, desc="Evaluating")):
		...
		if test_question:
			...
			user_content = construct_user_content(datapoint, args=eval_args)
		...
		if model_args.model_type == "chatgpt":
			response: ChatCompletion = model_pipeline.chat.completions.create(
				...
				messages=prompt_icl,
				max_tokens=XLWIC_MAX_NEW_TOKENS,
				temperature=None,
				top_p=None,
				seed=eval_args.seed
				)
			...
			response = model_pipeline(
				prompt_icl,
				max_new_tokens=XLWIC_MAX_NEW_TOKENS,
				do_sample=False,
				temperature=None,
				top_p=None,
		...
		extracted_label = extract_binary_response(response=model_response, lang_code=eval_args.lang_code)
		true_label = datapoint["label"]
		is_correct = extracted_label == true_label
		if is_correct:
			correct += 1
		...
		res["extracted_label"] = extracted_label
...
if __name__ == "__main__":
	...
	train_data_dir = os.path.join(grandparent_dir, 'data/XLWIC/xlwic_json_balanced/val')
	test_data_dir = os.path.join(grandparent_dir, 'data/XLWIC/xlwic_json_balanced/test')
	...
	_, high_resource_langs = get_low_high_resource_langs(XLWIC_LANGS)
