...
def main_eval(icl_mode: str) -> None:
    ...
    for dataset, (train_data, test_data, langs) in eval_data.items():
        ...
        low_resource_langs, high_resource_langs = get_low_high_resource_langs(langs)
        high_resource_langs.remove('en')
        
        eval_args.train_data = train_data
        eval_args.train_dataset_size = train_dataset_size

        if icl_mode == "english":
            eval_args.high_resource_langs = ["english"]
            eval_args.icl_mode = 'english'
        elif icl_mode == "high_resource":
            eval_args.high_resource_langs = high_resource_langs
            eval_args.icl_mode = 'multilingual'
        else:
            eval_args.high_resource_langs = low_resource_langs
            eval_args.icl_mode = 'multilingual'

        for _ in range(4): # 4 * 250 = 1000 total prompts
            # dummy data, won't be used when `test_question=False`
            eval_args.lang_code = 'en'
            eval_args.data_split = test_data['en']
...
if eval_args.eval_dataset == 'mgsm':
    for icl_mode in ["english", "low_resource", "high_resource"]:
